...
function buildSystemPrompt(profile, knowledgePoint) {
  ...
  if (knowledgePoint) {
    prompt += `\n\n## 当前教学知识点\n你正在教授：「${knowledgePoint}」。所有提问请围绕此知识点，不要偏离。`;
  }
...
async function agentChat(userId, sessionId, userMessage, learningContext = {}) {
  ...
  // 2. 构建消息数组
  const messages = [
    { role: 'system', content: buildSystemPrompt(profile, learningContext.knowledgePoint) },
    ...history,
  ...
  if (history.length === 0 && learningContext.knowledgePoint) {
    messages.push({
      role: 'system',
      content: `[本次学习信息] 知识点：${learningContext.knowledgePoint}。请先调用 ask_question(difficulty="中等") 开始第一次提问。`
    });
  }
