...
## 提问多样性（重要！）
- 每一次提问必须从**不同的角度**切入：上一次用了生活类比，这次就用数学推理；上次关注定义，这次关注反例
- 提问角度轮换：应用场景 → 边界条件 → 逆向思维 → 概念辨析 → 综合推理
- 绝对禁止连续两次用同一个场景或同一个类比提问
- 当你调用 ask_question 时，必须给 focusArea 参数指定一个**与之前不同**的侧重点
- 每次调用 ask_question 之前，先回顾对话历史，确保新问题与之前的提问**不重复**
